...
///
UFormattable* ufmt_open_74(UErrorCode* status);

///
void ufmt_close_74(UFormattable* fmt);

///
UFormattableType ufmt_getType_74(const(UFormattable)* fmt, UErrorCode* status);

///
UBool ufmt_isNumeric_74(const(UFormattable)* fmt);

///
UDate ufmt_getDate_74(const(UFormattable)* fmt, UErrorCode* status);

///
double ufmt_getDouble_74(UFormattable* fmt, UErrorCode* status);

///
int ufmt_getLong_74(UFormattable* fmt, UErrorCode* status);

///
long ufmt_getInt64_74(UFormattable* fmt, UErrorCode* status);

///
const(void)* ufmt_getObject_74(const(UFormattable)* fmt, UErrorCode* status);

///
const(UChar)* ufmt_getUChars_74(UFormattable* fmt, int* len, UErrorCode* status);

///
int ufmt_getArrayLength_74(const(UFormattable)* fmt, UErrorCode* status);

///
UFormattable* ufmt_getArrayItemByIndex_74(UFormattable* fmt, int n, UErrorCode* status);

///
const(char)* ufmt_getDecNumChars_74(UFormattable* fmt, int* len, UErrorCode* status);
